...
void * clnt_connection(void *arg)
{
		...
		while(1)
		{
				...
				if((!strcmp(msg_info.to, "A") || !strcmp(msg_info.to, "B") || !strcmp(msg_info.to, "C") || !strcmp(msg_info.to, "D")) && !strcmp(msg_info.from, "BOSS"))
				{
					sprintf(pArray[1],"%s\n", select_msg);
				}
...
void send_msg(MSG_INFO * msg_info, CLIENT_INFO * first_client_info)
{
		int i=0;
		char strBuff[BUF_SIZE*2]={0};
		msg_info->to[strcspn(msg_info->to, "\r\n")] = 0;
		...
		else if(strcmp(msg_info->to, "A") && strcmp(msg_info->to, "B") && strcmp(msg_info->to, "C") && strcmp(msg_info->to, "D") && !strcmp(msg_info->from, "BOSS"))
		{
			char store[30];
			sleep(1);
			strcpy(select_msg, msg_info->to);
			strcpy(store, "Select Store\n");
			write(msg_info->fd, store, strlen(store));
			selectFlag = 1;
		}
		else if((!strcmp(msg_info->to, "A") || !strcmp(msg_info->to, "B") || !strcmp(msg_info->to, "C") || !strcmp(msg_info->to, "D")) && selectFlag == 1 && !strcmp(msg_info->from, "BOSS"))
		{
			selectFlag = 0;
			sprintf(msg_info->msg, "[%s]%s\n", msg_info->from, select_msg);
